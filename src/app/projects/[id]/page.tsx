      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className={cn("rounded-full border px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.12em]", milestone.category === "bootstrap" ? "border-sky-200 bg-sky-50 text-sky-700" : "border-red-200 bg-red-50 text-red-700")}>{milestone.category === "bootstrap" ? "Bootstrap phase" : "Delivery phase"}</span>
          ...
          <p className="mt-2 text-xs leading-5 text-zinc-500">
            {milestone.doneTasks} of {milestone.totalTasks} {milestone.category === "bootstrap" ? "bootstrap" : "delivery"} tasks complete · {milestone.queuedTasks || 0} queued · {milestone.runningTasks || 0} running · {milestone.blockedTasks || 0} blocked
            {milestone.hiddenBootstrapTasks ? ` · ${milestone.hiddenBootstrapTasks} hidden bootstrap task${milestone.hiddenBootstrapTasks === 1 ? "" : "s"} excluded from delivery counts` : ""}
          </p>
        </div>
...
export default function ProjectDetailPage() {
  ...
  const { project, teams, milestones = [], tasks, recentSignals = [], events = [], stats, deliveryIntegrity, truth } = data;
  const bootstrapSprintIds = getBootstrapSprintIds(milestones.map((milestone) => ({ id: milestone.id, auto_generated: milestone.autoGenerated })));
            ...
            <div className={cn("rounded-2xl border p-4", statusTone.surface)}>
              ...
              <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                ...
                <div className="space-y-1 text-sm text-zinc-500 sm:text-right">
                  ...
                  {truth ? <div>{truth.counts.jobs.queued} queued jobs · {truth.counts.jobs.running} running jobs</div> : null}
                </div>
              </div>
            ...
            <div className="rounded-2xl border border-zinc-200 bg-white p-4 text-sm text-zinc-500">
              ...
              {truth ? (
                <div className="mt-2 space-y-2 leading-6">
                  <p>
                    {truth.counts.delivery.total} delivery task{truth.counts.delivery.total === 1 ? "" : "s"} and {truth.counts.bootstrap.total} bootstrap task{truth.counts.bootstrap.total === 1 ? "" : "s"} are visible.
                    {teams.length > 0 ? ` ${teams.length} team${teams.length === 1 ? " is" : "s are"} attached to delivery.` : ""}
                  </p>
                  <p>
                    Execution: {truth.counts.delivery.queued} queued, {truth.counts.delivery.running} running, {truth.counts.delivery.done} done, {truth.counts.delivery.blocked} blocked delivery tasks.
                  </p>
                </div>
              ) : (
          ...
          <Section title="Task board" description="Keep work moving without opening every task.">
            {truth ? (
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50 px-4 py-3 text-sm text-zinc-600">
                Delivery board shows seeded, running, blocked, and done work using current task status. Bootstrap tasks stay visible here, but headline progress only counts delivery tasks.
              </div>
            ) : null}
            {tasks.length === 0 ? (
              <EmptySectionState
                              ...
                              <div className="flex items-start justify-between gap-3">
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="mt-2 flex flex-wrap gap-2">
                                    ...
                                    {isBootstrapTask(task, bootstrapSprintIds) ? <span className="rounded-full border border-sky-200 bg-sky-50 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.14em] text-sky-700">Bootstrap</span> : <span className="rounded-full border border-red-200 bg-red-50 px-2.5 py-1 text-[10px] font-medium uppercase tracking-[0.14em] text-red-700">Delivery</span>}
