  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="mt-1 text-sm text-zinc-500">Choose a task type first. This V1 flow only exposes the fields that matter for that type.</p>

        <div className="mt-5 space-y-5">
          <section>
            <div className="flex items-center justify-between gap-3">
              <p className="text-sm font-medium text-zinc-900">1. Choose task type</p>
              {taskType ? (
            ...
            {!taskType ? (
              <div className="mt-4 rounded-xl border border-dashed border-zinc-300 bg-zinc-50 px-4 py-5 text-sm text-zinc-500">
                Select a type to unlock its structured fields.
              </div>
            ) : null}
          </section>

          {taskType && config && routing ? (
            <>
              ...
              <section className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-zinc-700">3. {config.goalLabel || "What should this task accomplish?"}</label>
                  <input value={taskGoal} onChange={(e) => setTaskGoal(e.target.value)} placeholder={config.goalPlaceholder || "Describe the outcome"} className="mt-1 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" />
                </div>
                <div>
